...
def print_trace(trace):
    print("\nPHASE 6 PIPELINE EXECUTION TRACE")
    print("=" * 90)
...
def main():
    parser = argparse.ArgumentParser(description="Test Phase 5 pipeline using trained DistilBERT sentiment model only.")
    ...
    print("\nPHASE 6: TRANSFORMER PIPELINE TEST")
    print("=" * 90)
    ...
    pipeline = MultiDomainReviewAnalysisPipeline(
        ...
        use_rag=False,
        output_dir="outputs/phase6_transformer_pipeline",
    )
    ...
    print("=" * 90)
    print(review_df["predicted_sentiment"].value_counts())

    print("\nREVIEW-LEVEL PREVIEW")
    print("=" * 90)
    preview_cols = [
        "domain", "rating", "sentiment_label", "predicted_sentiment",
        "sentiment_confidence", "primary_issue", "trust_score", "risk_level"
    ]
    preview_cols = [c for c in preview_cols if c in review_df.columns]
    print(review_df[preview_cols].head(12))

    print("\nENTITY SUMMARY PREVIEW")
    print("=" * 90)
    summary_cols = ["domain", "entity_type", "entity_name", "total_reviews", "average_trust_score", "overall_risk_level", "top_issues"]
    summary_cols = [c for c in summary_cols if c in entity_df.columns]
    print(entity_df[summary_cols].head(10))

    print("\nPHASE 6 PIPELINE TEST COMPLETE")
    print("Outputs saved in: outputs/phase6_transformer_pipeline/")
